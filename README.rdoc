...
When running in development mode, by default real api calls are not made. Dummy responses are returned from the DummyResponse class. Set the REMOTE_TEST environment variable to "1" to make real api calls when in development mode like so:

env REMOTE_TEST=1 script/server
